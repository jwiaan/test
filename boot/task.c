...
struct {
	uint32_t _, *esp0;
	uint16_t ss0, __[46], io;
} tss = {.ss0 = 16,.io = 103 };

#define save(m) asm("pusha;push %ds;push %es");asm("mov %%esp,%0;out %1,$0x20":"=m"(m):"a"((uint8_t)0x20))
#define load(m) asm("mov %0,%%esp;pop %%es;pop %%ds;popa;pop %%ebp;iret"::"m"(m))
...
void setup_idt(void)
{
	...
	*(uint64_t **) (idtr + 1) = idt;
	asm("ltr %0;lidt %1;sti"::"r"((uint16_t) 40), "m"(idtr));
}
...
void init_task(int i)
{
	...
	*(int *)esp = i;
	task[i].stack[0] = 35;
	task[i].stack[1] = 35;
	task[i].stack[11] = (uint32_t) start;
	task[i].stack[12] = 27;
	task[i].stack[13] = 0x202;
	task[i].stack[14] = esp - 4;
	task[i].stack[15] = 35;
